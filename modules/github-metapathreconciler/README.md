...
| <a name="input_extra_pr_event_types"></a> [extra\_pr\_event\_types](#input\_extra\_pr\_event\_types) | Additional CloudEvents PR event types to subscribe to, beyond this module's<br/>default set (pull\_request, pull\_request\_review, pull\_request\_review\_comment,<br/>check\_run/check\_suite completed). Each entry has the same shape consumed by<br/>the cloudevents-workqueue filters, e.g. { type = "dev.chainguard.github.issue\_comment" }<br/>to also react to plain PR conversation comments.<br/><br/>Defaults to none, so existing consumers of this module see no change in<br/>subscribed event types. | <pre>list(object({<br/>    type   = string<br/>    action = optional(string)<br/>  }))</pre> | `[]` | no |
